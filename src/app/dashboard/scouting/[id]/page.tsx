...
import { Pencil, Trash2, ChevronLeft, Phone, Save } from 'lucide-react';
    ...
    <div className="space-y-5 max-w-5xl animate-fade-in-up">
      ...
      <div className="card p-6">
        <div className="flex flex-col md:flex-row gap-5 md:items-start md:justify-between">
          <div className="flex gap-5 items-start">
            <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-brand-100 to-brand-300 flex items-center justify-center text-4xl overflow-hidden shadow-glow-orange border-2">
              {recruit.photo_url ? (
            ...
            <div>
              ...
              <div className="text-sm mt-1 space-y-0.5" style={{ color: 'var(--text-secondary)' }}>
                ...
                {recruit.whatsapp_number && (
                  <div className="flex items-center gap-1 text-green-700 font-semibold"><Phone size={14} /> {recruit.whatsapp_number}</div>
                )}
              </div>
              <div className="flex gap-2 mt-2">
                <Badge variant="brand">{recruit.status}</Badge>
                {recruit.scout_recommendation && (
                  <Badge variant={recruit.scout_recommendation === 'sign' ? 'success' : recruit.scout_recommendation === 'pass' ? 'danger' : 'warning'}>
                    {recruit.scout_recommendation}
                  </Badge>
                )}
              </div>
      ...
      {recruit.notes && (
        <section className="card p-6">
          <h2 className="font-bold text-lg mb-2" style={{ color: 'var(--text-primary)' }}>Scout notes</h2>
          <p className="whitespace-pre-line" style={{ color: 'var(--text-secondary)' }}>{recruit.notes}</p>
        </section>
      )}

      {/* Scouting Evaluation with sliders */}
      <section className="card p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-lg" style={{ color: 'var(--text-primary)' }}>Scouting Evaluation</h2>
          <Button size="sm" variant="secondary" loading={saving} onClick={() => {}}><Save size={14} /> Auto-saves</Button>
        </div>
        <ScoutingEvaluation
          initial={{
            scout_tech_ball_control: recruit.scout_tech_ball_control ?? 5,
            scout_tech_passing: recruit.scout_tech_passing ?? 5,
            scout_tech_shooting: recruit.scout_tech_shooting ?? 5,
            scout_tech_dribbling: recruit.scout_tech_dribbling ?? 5,
            scout_phy_speed: recruit.scout_phy_speed ?? 5,
            scout_phy_strength: recruit.scout_phy_strength ?? 5,
            scout_phy_endurance: recruit.scout_phy_endurance ?? 5,
            scout_phy_agility: recruit.scout_phy_agility ?? 5,
            scout_tac_positioning: recruit.scout_tac_positioning ?? 5,
            scout_tac_awareness: recruit.scout_tac_awareness ?? 5,
            scout_tac_decision: recruit.scout_tac_decision ?? 5,
            scout_psy_confidence: recruit.scout_psy_confidence ?? 5,
            scout_psy_leadership: recruit.scout_psy_leadership ?? 5,
            scout_psy_composure: recruit.scout_psy_composure ?? 5,
            scout_psy_work_ethic: recruit.scout_psy_work_ethic ?? 5,
          }}
          onChange={saveScores}
        />
      </section>
