...
export default function PlayerCard({ p }: { p: Player }) {
  return (
    <Link href={`/dashboard/players/${p.id}`} className="card p-4 card-hover flex gap-3 items-center group">
      <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-brand-100 to-brand-200 dark:from-brand-900/30 dark:to-brand-800/30 flex items-center justify-center text-2xl overflow-hidden shrink-0 border" style={{ borderColor: 'var(--border-main)' }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>
          {p.position} · {p.age_group || 'No group'} · Age {ageFromDOB(p.date_of_birth)}
